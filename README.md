...
El cliente vive en `app_flutter/`. Se ha ejecutado en un dispositivo Android físico y obtiene la URL del backend desde la variable de compilación `API_BASE_URL`. Su pantalla de comprobación consume `GET /api/categorias`, muestra la cantidad y los nombres recibidos, e informa errores de configuración o conexión.
...
Conecte el dispositivo Android por USB, habilite las Opciones de desarrollador y la Depuración USB, y compruebe que Flutter lo detecta:
...
## Configuración del entorno móvil Flutter
...
El resultado esperado de `flutter doctor -v` es `No issues found`, y `flutter devices` debe mostrar el dispositivo disponible. Instale las dependencias desde `app_flutter/` con `flutter pub get` y ejecute la aplicación con el comando y la variable `API_BASE_URL` indicados en la sección anterior.

La excepción de Android en `android/app/src/main/res/xml/network_security_config.xml` autoriza HTTP únicamente para `localhost` durante el desarrollo. No debe ampliarse a cualquier dominio ni utilizarse como configuración general de producción.
